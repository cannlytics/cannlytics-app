...
class Scrapboard<T> extends StatefulWidget {
  ...
  final void Function(ScrapData<T>, List<ScrapData<T>> boxes)
      onSelectionChanged;
...
class ScrapboardState<T> extends State<Scrapboard<T>>
  ...
  Widget build(BuildContext context) {
    ...
    bool showItemControls =
        _selected.length == 1 && widget.itemControlsBuilder != null;
    return Scaffold(
      body: Stack(
        children: [
                ...
                child: Stack(
                  ...
                  children: [
                    ..._tmpBoxes.map((boxData) {
                      ...
                      bool showControls =
                          isSelected && _selectedBoxIds.length == 1;
  ...
  List<ScrapData<T>> _getSelectedBoxInstances() => _tmpBoxes
      .where((b) => _selectedBoxIds.contains(widget.idBuilder(b)))
      .toList();
  ...
  void _handleCornerDragged(ScrapData<T> boxData, Offset delta) {
    ...
    if (_lockAspectOnResize || isLockForced) {
      ...
      delta = Offset(useHz ? delta.dx : delta.dy * boxData.aspect,
          useHz ? delta.dx / boxData.aspect : delta.dy);
    }
    Size origSize = boxData.size;
    boxData.size += delta;
    boxData.size = _clampBoxSize(boxData); // Enforce min size
    delta = Offset(boxData.size.width - origSize.width,
        boxData.size.height - origSize.height);
