...
import 'package:cannlytics_app/_utils/data_utils.dart';
...
import 'package:cannlytics_app/views/scrap_popup_editor/scrap_popup_editor.dart';
import 'package:cannlytics_app/views/scrapboard_editor_page/placed_scrap_renderer.dart';
import 'package:cannlytics_app/views/scrapboard_editor_page/scrapboard/scrap_data.dart';
import 'package:cannlytics_app/views/scrapboard_editor_page/scrapboard/scrapboard.dart';
...
class NetworkedScrapboard extends StatefulWidget {
  ...
  final void Function(List<ScrapData<PlacedScrapItem>> boxes)
      onSelectionChanged;
}
...
class _NetworkedScrapboardState extends State<NetworkedScrapboard> {
  ...
  Widget build(BuildContext context) {
    ...
    scrapsList = DataUtils.sortListById(
        (scrapsList ?? []), _currentPage?.boxOrder ?? []);
    // Create ScrapBoxData from our PlacedScraps, that the Scrapboard can work with.
    List<ScrapData<PlacedScrapItem>> boxes =
        scrapsList.map((item) => item.toBoxData()).toList();
    // Main Stack
    return Stack(
      children: [
        ...
        Scrapboard<PlacedScrapItem>(
          ...
          itemBuilder: (item) {
            int _selectedIndex = _selectedBoxes
                .indexWhere((b) => b.data.documentId == item.documentId);
          ...
          lockAspectForItem: (item) =>
              item.data.contentType == ContentType.Emoji,
  ...
  void _handleBoxReorder(ScrapPageData scrapPageData,
      List<ScrapData<PlacedScrapItem>> boxList) async {
    List<String> boxOrder =
        boxList.map((element) => element.data.documentId).toList();
