...
class BooksModel extends EasyNotifier {
  ...
  set books(List<ScrapBookData> value) {
    ...
    value.sort((item1, item2) =>
        item1.lastModifiedTime > item2.lastModifiedTime ? -1 : 1);
    notify(() => _books = value);
  }
  ...
  void replaceBook(ScrapBookData value) {
    ...
    bool equals(ScrapBookData b1, ScrapBookData b2) =>
        b1?.documentId == b2?.documentId;
  ...
  set currentBookScraps(List<ScrapItem> value) =>
      notify(() => _currentBookScraps = value);
  ...
  void removeBookScrapById(String documentId) => notify(() {
        ...
        currentBookScraps = List.from(currentBookScraps)
          ..removeWhere((scrap) => scrap.documentId == documentId);
      });

  void replacePage(ScrapPageData value) {
    if (currentBookPages == null) return;
    bool equals(ScrapPageData p1, ScrapPageData p2) =>
  ...
  void removePageScrapById(String documentId) {
    if (currentPageScraps == null) return;
    currentPageScraps = List.from(currentPageScraps)
