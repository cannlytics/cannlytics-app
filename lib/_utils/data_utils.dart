...
class DataUtils {
  static List<T> sortListById<T extends FirebaseDoc>(
      List<T> pages, List<String> pageIds) {
    if (pageIds?.isEmpty ?? true) return pages;
    List<_IndexPair> listIndices = [];
    List<T> sortedList = [];
    for (int i = 0; i < pages.length; ++i) {
      int elementOrder =
          pageIds.indexWhere((pageId) => pageId == pages[i].documentId);
      listIndices
          .add(_IndexPair(elementOrder != -1 ? elementOrder : 1000000000, i));
    }
