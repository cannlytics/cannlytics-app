...
class StyledToggleSwitch extends StatefulWidget {
  StyledToggleSwitch(
      ...
      this.materialIcon2})
      : super(key: key) {
    assert((icon1 != null || materialIcon1 != null) &&
        (icon2 != null || materialIcon2 != null));
  }
  final bool value;
...
class _StyledToggleSwitchState extends State<StyledToggleSwitch> {
  ...
  Widget build(BuildContext context) {
    ...
    BoxDecoration dec(Color c, double b) =>
        BoxDecoration(color: c, borderRadius: BorderRadius.circular(b));
    double btnWidth = 32;
    return SizedBox(
      width: btnWidth * 2,
      height: btnWidth,
        ...
        children: [
          ...
          Positioned.fill(
            ...
            children: [
              _BtnWithTooltip(
                onPressed:
                    !widget.value ? null : () => widget.onToggled?.call(false),
                child: _buildIcon(widget.icon1, widget.materialIcon1),
                toolTip: widget.tooltip1,
              ),
              _BtnWithTooltip(
                onPressed:
                    !widget.value ? () => widget.onToggled?.call(true) : null,
          ...
          AnimatedContainer(
            ...
            child: IgnorePointer(
              child: Container(
                ...
                height: 32,
                decoration: dec(theme.surface1, Corners.sm)
                    .copyWith(boxShadow: Shadows.universal),
                ...
                child: AnimatedSwitcher(
                  ...
                  child: Container(
                    key: ValueKey(widget
                        .value), // key is required to trigger AnimatedSwitcher
                    child: (widget.value)
                        ? _buildIcon(widget.icon2, widget.materialIcon2,
                            color: theme.accent1)
                        : _buildIcon(widget.icon1, widget.materialIcon1,
                            color: theme.accent1),
                  ),
                ),
              ),
...
class _BtnWithTooltip extends StatelessWidget {
  ...
  const _BtnWithTooltip({Key key, this.onPressed, this.toolTip, this.child})
      : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Expanded(
