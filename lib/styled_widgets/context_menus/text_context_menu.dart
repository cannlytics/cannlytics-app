...
class TextContextMenu extends BaseContextMenu {
  ...
  void _handleCopyPressed() async {
    String value =
        controller == null ? data : controller.selection.textInside(data);
...
extension TextFormFieldContextExtension on TextFormField {
  Widget addContextMenu(String data,
      {TextEditingController controller, bool obscuredText = false}) {
    return ContextMenuRegion(
      contextMenu: TextContextMenu(
          data: data, controller: controller, obscuredText: obscuredText),
      child: this,
    );
  }
}
